/* =====================================
   ABERTURA DO SITE
   Sempre começa no topo, sem abrir a carta automaticamente.
===================================== */
...
const botaoSurpresa = document.getElementById("surpriseBtn");
const surpresa = document.getElementById("secret");

botaoSurpresa.addEventListener("click", function () {
    surpresa.classList.toggle("open");

    if (surpresa.classList.contains("open")) {
        botaoSurpresa.textContent = "❤️ Eu te amo";
    } else {
        botaoSurpresa.textContent = "Abrir surpresa ❤️";
    }
});
...
inputs.forEach(function (input) {
    input.addEventListener("change", function (evento) {
        ...
        leitor.onload = function (e) {
            ...
            if (placeholder) {
                placeholder.innerHTML = "<span>❤️</span><p>Foto adicionada</p>";
...
function criarCoracao() {
    ...
    coracao.className = "float-heart";
    coracao.textContent = Math.random() > 0.35 ? "♥" : "♡";
    coracao.style.left = Math.random() * 100 + "vw";
    coracao.style.animationDuration = (5 + Math.random() * 6) + "s";
